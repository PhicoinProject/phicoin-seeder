...
//                printf("%s:\t added address %s (#%i)\n", ToString(you).c_str(), addr.ToString().c_str(), (int) (vAddr->size()));
    ...
    bool ProcessMessages()
    {
        ...
        do
        {
            ...
            {
                if ((vRecv.end() - pstart) && !memcmp(&pstart[0], pchMessageStart, vRecv.end() - pstart))
                {
            ...
            if (vRecv.GetVersion() >= 209)
            {
                ...
                unsigned int nChecksum = 0;
                memcpy(&nChecksum, &hash, sizeof(nChecksum));
                if (nChecksum != hdr.nChecksum) return false;
            }
...
public:
    ...
    bool Run()
    {
        ...
        int64 now;
        while (doneAfter == 0 || (now = time(NULL)) < doneAfter)
        {
            char pchBuf[0x10000];
            fd_set set;
            FD_ZERO(&set);
            ...
            int ret = select(sock + 1, &set, NULL, NULL, &wa);
            if (ret != 1)
            {
                if (!doneAfter) found = true;
                break;
            }
            int nBytes = recv(sock, pchBuf, sizeof(pchBuf), 0);
            if (nBytes > 0)
            {
                vRecv.insert(vRecv.end(), pchBuf, pchBuf + nBytes);
                found = ProcessMessages();
            } else
            {
                if (doneAfter == 0) found = true;
                break;
            }
            Send();
        }
