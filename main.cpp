...
bool fTestNet = false;
bool fRegTest = false;
bool gRequireReachable = true;
int gDumpIntervalSec = 0;
...
public:
    ...
    int fWipeBan;
    int fWipeIgnore;
    int fAllowUnroutable;
    int nDumpIntervalSec;
    const char *mbox;
    const char *ns;
    const char *host;
    const char *tor;
    ...
    CDnsSeedOpts()
            : nThreads(96), nDnsThreads(4), nPort(53), fAllowUnroutable(false), nDumpIntervalSec(0),
              mbox(NULL), ns(NULL), host(NULL), tor(NULL), fUseTestNet(false), fUseRegTest(false),
              fWipeBan(false), fWipeIgnore(false), ipv4_proxy(NULL), ipv6_proxy(NULL)
    {}

    void ParseCommandLine(int argc, char **argv)
    {
        static const char *help = "Phicoin-seeder\n"
                                  ...
                                  "-S <seconds>    Dump interval in seconds (fixed). If not set, uses exponential backoff\n"
                                  "-o <ip:port>    Tor proxy IP/Port\n"
                                  "-i <ip:port>    IPV4 SOCKS5 proxy IP/Port\n"
                                  "-k <ip:port>    IPV6 SOCKS5 proxy IP/Port\n"
                                  "-w f1,f2,...    Allow these flag combinations as filters\n"
                                  "--allow-unroutable  Accept non-routable addresses (disable reachability filter)\n"
                                  "--testnet       Use testnet\n"
                                  "--regtest       Use regtest\n"
        ...
        while (1)
        {
            static struct option long_options[] = {
                    ...
                    {"dump-interval", required_argument, 0,         'S'},
                    {"onion",      required_argument, 0,            'o'},
                    {"proxyipv4",  required_argument, 0,            'i'},
                    {"proxyipv6",  required_argument, 0,            'k'},
                    {"filter",     required_argument, 0,            'w'},
                    {"allow-unroutable", no_argument, &fAllowUnroutable, 1},
            ...
            int option_index = 0;
            int c = getopt_long(argc, argv, "h:n:m:t:p:d:S:o:i:k:w:?:", long_options, &option_index);
            if (c == -1) break;
            switch (c)
            {
                case 'h':
                ...
                {
                    int n = strtol(optarg, NULL, 10);
                    if (n > 0 && n < 86400) nDumpIntervalSec = n;
                    break;
                }
...
extern "C" void *ThreadDumper(void *)
{
    ...
    do
    {
        if (gDumpIntervalSec > 0) {
            Sleep(gDumpIntervalSec * 1000);
        } else {
...
// PHICOIN upstream DNS seeds (see upstream src/chainparams.cpp)
...
static const string testnet_seeds[] = {
        "seed1.test.phicoin.net",
        "seed2.test.phicoin.net",
        ""
};
...
extern "C" void *ThreadSeeder(void *)
{
    ...
    {
        // No hardcoded onion seed for PHICOIN.
    }
    do
    {
...
int main(int argc, char **argv) {
    ...
    gRequireReachable = !opts.fAllowUnroutable;
    gDumpIntervalSec = opts.nDumpIntervalSec;
    ...
    if (opts.fUseTestNet) {
        ...
        // Upstream chainparams.cpp testnet section appears to have a typo (overwrites [0] and omits [3]).
        // We interpret the intended testnet magic as: 0x84 'P' 'H' 'I'.
    ...
    if (opts.fUseRegTest) {
        ...
        // Keep regtest distinct from mainnet.
        // Upstream chainparams.cpp regtest section appears to have the same typo; use the same 0x84 'P' 'H' 'I'.
        pchMessageStart[0] = 0x84;
        pchMessageStart[1] = 0x50; // 'P'
        pchMessageStart[2] = 0x48; // 'H'
        pchMessageStart[3] = 0x49; // 'I'
