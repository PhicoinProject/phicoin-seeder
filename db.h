...
// Match PHICOIN minimum peer protocol version (see upstream src/version.h)
#define REQUIRE_VERSION 80000

// Runtime flags (set in main.cpp).
extern bool gRequireReachable;

static inline int GetRequireHeight(const bool testnet = fTestNet, const bool regtest = fRegTest)
{
    // PHICOIN: do not enforce a minimum chain height by default.
    // If you want to make the seed stricter, adjust these thresholds.
...
public:
    ...
    bool IsGood() const
    {
        // Do not require default port or NODE_NETWORK; accept more peers.
        if (gRequireReachable && !ip.IsRoutable()) return false;
        //printf("3\n");
...
public:
    ...
    void GetStats(CAddrDbStats &stats)
    {
        SHARED_CRITICAL_BLOCK(cs)
        {
            ...
            // If no tried nodes yet, show age as 0 instead of epoch seconds.
            if (ourId.empty()) {
                stats.nAge = 0;
            } else {
